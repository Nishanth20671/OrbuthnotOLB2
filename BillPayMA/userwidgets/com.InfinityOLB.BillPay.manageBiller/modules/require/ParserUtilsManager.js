...
define(function () {
   ...
   *
   * @param {String} value
   * @paaram {Integer String} responseArrayIndex
   * 
   * @return {String} - value for string
   */
  ParserUtilsManager.prototype.getParsedValue = function(value, responseArrayIndex){
    var JSONValue = {}
    try{
    ...
    return this.checkPatternPresent(value, responseArrayIndex);
  };
   ...
   * @return {String} - value for string
   */
  ParserUtilsManager.prototype.checkPatternPresent = function(text, responseArrayIndex){ 
    var parseStr = text;
    var delimiters="i$";
    ...
    if(decoratorArray){
      for(var i = 0; i < decoratorArray.length; i++){
        parseStr = parseStr.replace(decoratorArray[i],this.identifyPattern(decoratorArray[i], responseArrayIndex));
      }
    }
    return parseStr;
  };
   ...
   * @return {String} - value for string
   */
  ParserUtilsManager.prototype.identifyPattern = function(text, responseArrayIndex){
    ...
    //Check if text is having a unicode
      ...
      if(unicode == "c"){
        ...
        return this.getResponseValue(text.substring(valueStartIndex,text.length-1),unicode, responseArrayIndex);
      }
    }
  };
   ...
   * @return {String} - value for string
   */
  ParserUtilsManager.prototype.getResponseValue = function(text,unicode, responseArrayIndex){
    var responseValue = "";
    if(responseArrayIndex === null || responseArrayIndex === undefined || responseArrayIndex === ""){
      try{
      ...
      try{
        responseValue = this.unicode[unicode][responseArrayIndex][text];
        if(responseValue !== "" && responseValue !== undefined && responseValue !== null){
          return responseValue;
        }
      }catch(e){
