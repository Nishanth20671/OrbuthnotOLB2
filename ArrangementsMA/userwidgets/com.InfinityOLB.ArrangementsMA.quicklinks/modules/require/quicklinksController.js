define(['./EntitlementUtils'], function(EntitlementUtils) {
  ...
  return {
    ...
    accSetFocus: function(){
      var self = this;
      self.view["btnRowLink1"].setFocus(true);
    },
...
//       if(breakpoint === "1366dp") {
        for(var i = 1; i <= 10; i++) {
          self.view["btnRowLink" + i].skin = self._sknLink;
...
//         self._context["entitlement"] = {};
    ...
    setLinks: function() {
      ...
      try {
        for(var i = 1; i <= 10; i++) {
          ...
          var accessibility = self["_link" + i + "accessibilityConfig"];
          if(link !== "") {
            self.view["btnRowLink" + i].text = link;
            self.view["btnRowLink" + i].accessibilityConfig = accessibility;
          } else {
    ...
    drawLinks: function() {
      ...
      try {
        for(var i = 1; i <= 10; i++) {
          self.view["btnRowLink" + i].isVisible = false;
        ...
        for(; j <= self._numberOfLinks; j++) {
          ...
          if(self.EntitlementUtils.isEntitled(action.entitlement)) {
            self.view["btnRowLink" + j].isVisible = true;
    ...
    initActions: function() {
      ...
      try {
        for(var i = 1; i <= self._numberOfLinks; i++) {
          ...
          self.view["btnRowLink" + i].onClick = self._parentScope[action.method];
        }
      } catch(err) {
          var errObj = {
    ...
    overrideLinksFromContext: function() {
      ...
      try {
        ...
        for(; i <= self._numberOfLinks; i++) {
          ...
          self["_link" + i + "accessibilityConfig"] = context[i - 1]["accessibilityConfig"];
        }
        for(; i	 <= 10; i++) {
          self["_link" + i + "Text"] = "";
        }
    ...
    overrideActionsFromContext: function() {
      ...
      try {
        var context = self._context;
		//Temporary fix and need to revisit
		if(self._numberOfLinks > 10)
			self._numberOfLinks = 10;
        for(var i = 1; i <= self._numberOfLinks; i++) {
