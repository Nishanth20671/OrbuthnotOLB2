define(["CommonUtilities", "FormControllerUtility", "OLBConstants", "ViewConstants"], function (CommonUtilities, FormControllerUtility, OLBConstants, ViewConstants) {
    ...
    return {
        ...
        updateFormUI: function (viewModel) {
            if (viewModel.serverError) {
                ...
                if (viewModel.manageRecipients) {
                    ...
                    this.WireRecipientView(viewModel.manageRecipients.recipients);
                }
            }
        ...
        showServerError: function (msg) {
            this.view.rtxDowntimeWarning.text = msg;
            this.view.rtxDowntimeWarning.toolTip = msg;
            this.view.flxDowntimeWarning.setVisibility(true);
        },
        WireRecipientView: function (data) {
            var newdata = this.filterData(data.UnSelectedPayees);
            this.WireRecipientResponse = newdata;
            this.setEditRecipientView(newdata);
            if (data.UnSelectedPayees.length > 0 && newdata.length == 0) this.showNoPermission();
            else this.showManageTransferRecipients(newdata);
        },
        setEditRecipientView: function (data) {
            ...
            this.view.imgSortAccountName.onTouchEnd = function () {
                ...
                self.sortRecordsBasedOnParams("payeeName", self.recipientOrder);
            }
                    ...
                    if (viewModel[i] !== undefined) {
                        var dataObject = {
                            ...
                            "lblIcon": {
                                ...
                                "text": viewModel[i].type === "Individual" ? "s" : "r"
                            },
                            "lblAccNumber": {
        ...
        sortRecordsBasedOnParams: function (sortParam, sortOrder) {
            ...
            if (sortParam === "payeeName") {
                if (sortOrder === "ASC") {
                    record.sort(function (name1, name2) {
                        if (name1 && name2) return name1.payeeName.localeCompare(name2.payeeName)
                    })
                }
                if (sortOrder === "DSC") {
                    record.sort(function (name1, name2) {
                        if (name1 && name2) return name2.payeeName.localeCompare(name1.payeeName)
                    })
                }
            }
            ...
            if (sortParam === "AccountType") {
                if (sortOrder === "ASC") {
                    record.sort(function (name1, name2) {
                        if (name1 && name2) return name1.wireAccountType.localeCompare(name2.wireAccountType)
                    })
                }
                if (sortOrder === "DSC") {
                    record.sort(function (name1, name2) {
                        if (name1 && name2) return name2.wireAccountType.localeCompare(name1.wireAccountType)
                    })
                }
            }
            if (sortParam === "AddedBy") {
                if (sortOrder === "ASC") {
                    record.sort(function (name1, name2) {
                        if (name1 && name2) return name1.AddedBy.localeCompare(name2.AddedBy)
                    })
                }
                if (sortOrder === "DSC") {
                    record.sort(function (name1, name2) {
                        if (name1 && name2) return name2.AddedBy.localeCompare(name1.AddedBy)
                    })
                }
            }
        ...
        filterData: function (recipients) {
            ...
            for (var i = 0; i < recipients.length; i++) {
                if (recipients[i].wireAccountType.toLowerCase() === OLBConstants.BULK_WIRE_TRANSFER_TYPE.DOMESTIC && this.checkAllPermissions([OLBConstants.BULK_WIRE_PERMISSIONS.DOMESTIC_WIRE_TRANSFER_VIEW_RECEPIENT])) {
                    data.push(recipients[i]);
                } else if (recipients[i].wireAccountType.toLowerCase() === OLBConstants.BULK_WIRE_TRANSFER_TYPE.INTERNATIONAL && this.checkAllPermissions([OLBConstants.BULK_WIRE_PERMISSIONS.INTERNATIONAL_WIRE_TRANSFER_VIEW_RECEPIENT])) {
                    data.push(recipients[i]);
                }
            }
            return data;
        ...
        /** Onclick Back Button */
        NavigatetoEditTemplate: function () {
            var wireTransferModule = kony.mvc.MDAApplication.getSharedInstance().getModuleManager().getModule({
                "moduleName": "WireTransferNewUIModule",
                "appName": "WireTransferMA"
            });
            var templateID = wireTransferModule.presentationController.getBulkWireTemplateID();
            var param = {
                "bulkWireTemplateID": templateID
            }
            wireTransferModule.presentationController.editselectedRecipients = [];
            wireTransferModule.presentationController.showBulkWiretemplatesEditRecipients(param, false);
        },
        /** Onclick Add Button */
        AddRecipientstoTemplate: function () {
            ...
            if (selectedRecords.length > 0) {
                ...
                if (wireTransferModule.presentationController.isBWTAdditionOfRecordPossible() <= 0 || selectedRecipients.length > wireTransferModule.presentationController.isBWTAdditionOfRecordPossible()) {
                    ...
                    FormControllerUtility.showProgressBar(this.view);
                    wireTransferModule.presentationController.addAdditionalKeysInEditRecipientsRecord(self.selectedRecords, OLBConstants.RECIPIENT_CATEGORY.EXISTING_RECIPIENT);
                    wireTransferModule.presentationController.addExistingEditRecipients(self.selectedRecords);
                    wireTransferModule.presentationController.editselectedRecipients = [];
                    wireTransferModule.presentationController.UpdateBulkWireTemplateRecipients(self.view.id, self.selectedRecords);
                }
            }
        }
    };
