	...
	return {
		...
		setDateFormat: function (newDateFormat) {
		  ...
		  if(this.dateFormat.indexOf("M") === -1){
			mIndex = this.dateFormat.indexOf("m");
		  }
		  else{
			mIndex = this.dateFormat.indexOf("M");
		  }
		  if(this.dateFormat.indexOf("D") === -1){
			dIndex = this.dateFormat.indexOf("d");
		  }
		  else{
			dIndex = this.dateFormat.indexOf("D");
		  }
		  if(this.dateFormat.indexOf("Y") === -1){
			yIndex = this.dateFormat.indexOf("y");
		  }
		  else{
			yIndex = this.dateFormat.indexOf("Y");
		  }
			this.formatPos.mPos = mIndex;
			this.formatPos.dPos = dIndex;
			this.formatPos.yPos = yIndex;
