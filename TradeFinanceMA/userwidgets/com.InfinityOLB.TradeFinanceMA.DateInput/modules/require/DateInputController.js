define(function () {
	return {
		...
		setFocusSkin: function () {
			this.view.skin = "sknBorderE3E3E3";
		},
      ...
      setText: function(dateToSet) {
		dateToSet = dateToSet.split("/").map(p => p.length === 1 ? "0" + p : p).join("/");
        this.view.tbxDateInputKA.text = dateToSet;
        if(dateToSet == ""){
          this.view.lblEnteredDateKA.text = dateToSet;
          this.view.lblDatePlaceholderKA.text = this.dateFormat;
          return;
        }else if(dateToSet.length === 10)
          this.view.lblDatePlaceholderKA.text = this.dateFormat.slice(dateToSet.length);
        if(this.view.tbxDateInputKA.getSelection())
