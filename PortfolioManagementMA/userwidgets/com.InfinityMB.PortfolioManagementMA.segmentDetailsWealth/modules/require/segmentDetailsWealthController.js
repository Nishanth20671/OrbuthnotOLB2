define(['./ParserUtilsManager', './FormatUtils', './WealthDetailsListDAO'],function(ParserUtilsManager, FormatUtils, WealthDetailsListDAO) {
   ...
   //declaration for Fields keys & values in the group:Row Data
          ...
          this._totalValue = "";
          this._proposalDate = "";
          this._proposalDateVal = "";
          this._reviewTotal = "";
          this._reviewTotalVal = "";
          this._feesAndTax = "";
          this._feesAndTaxVal = "";
      ...
      defineSetter(this, 'featuresAndPermission', function (val) {
        ...
        this._featuresAndPermission = val;
            ...
            defineGetter(this, 'proposalDate', () => {
                return this._proposalDate;
            });
            defineSetter(this, 'proposalDate', value => {
                this._proposalDate = value;
            });
            defineGetter(this, 'proposalDateVal', () => {
                return this._proposalDateVal;
            });
            defineSetter(this, 'proposalDateVal', value => {
                this._proposalDateVal = value;
            });
            defineGetter(this, 'reviewTotal', () => {
                return this._reviewTotal;
            });
            defineSetter(this, 'reviewTotal', value => {
                this._reviewTotal = value;
            });
            defineGetter(this, 'reviewTotalVal', () => {
                return this._reviewTotalVal;
            });
            defineSetter(this, 'reviewTotalVal', value => {
                this._reviewTotalVal = value;
            });
            defineGetter(this, 'feesAndTax', () => {
                return this._feesAndTax;
            });
            defineSetter(this, 'feesAndTax', value => {
                this._feesAndTax = value;
            });
            defineGetter(this, 'feesAndTaxVal', () => {
                return this._feesAndTaxVal;
            });
            defineSetter(this, 'feesAndTaxVal', value => {
                this._feesAndTaxVal = value;
            });
        },
      ...
      getPreviousDaysText: function(){
        ...
        this.setError(err, "checkPermission");
            ...
            historyEndDate = scope.criteriaObjectValue.endDate;
               ...
               this.view.imgArrowProposal.onTouchEnd = this.imgArrowChange;
			  ...
			  this._requestParam=JSON.stringify(criteriaParams);
               ...
               if(this._topFlex == "Transactions"){
                 ...
                 this.view.flxReviewProposalValues.setVisibility(false);
               ...
               else if(this._topFlex == "ReviewProposals" || this._topFlex == "ReviewPastProposals"){
                  this.view.flxTimePeriod.setVisibility(false);
                 this.view.flxReviewProposalValues.setVisibility(true);
                 this.view.flxPortfolioValues.setVisibility(false);
                  this.view.flxOrderToggle.setVisibility(false);
                  this.view.flxSeperator1.setVisibility(false);
                 this.view.flxCashAccounts.setVisibility(false);
...
//                 }
   ...
   //this.criteriaObjectValue["startDate"] = "";
    ...
    this.criteriaObjectValue["endDate"] = historyEndDate;
               ...
               else if (this._topFlex === "ReviewPastProposals") {
                this.criteriaObjectValue["type"] = "pastProposal";
              }
              else if (this._topFlex === "ReviewProposals") {
                this.criteriaObjectValue["type"] = "orderProposal";
	            this.criteriaObjectValue["pastProposalId"] = "";
              }
            } catch (err) {
                var errorObj = {
     ...
     getCriteriaList: function() {
            ...
            try {
              ...
              if(this.criteriaObjectValue.searchByInstrumentName === " "){
                this.criteriaObjectValue.searchByInstrumentName="";
              }
	...
	onError: function(errorObj){
      ...
      { 
        var obj = JSON.parse(this._noRecordsFoundText);
          ...
          if( existingparam !== "" && existingparam !== 'undefined' && existingparam.type=="history")
          { 			
            errData = this.getFieldValue(obj["history"]);
          }
          else{
            errData = this.getFieldValue(obj["open"]);
          }
        //this.view.lblError.text = obj["error"];
      }
      else{
        //this.view.lblError.text = this._noRecordsFoundText;
        errData = this.getFieldValue(this._noRecordsFoundText);
      }
      if(errData)
      {
        this.view.lblError.text = errData;
      }
      else
      {
        this.view.lblError.text = kony.i18n.getLocalizedString("i18n.konybb.common.NoData");
      }
...
// ];
      ...
      processResponse: function(listResponse){
        try{
        if(this._topFlex === "ReviewPastProposals"){
          this._serviceResponseKey = "pastProposal";
          var responseKey = this._serviceResponseKey;
        }
        else if (this._topFlex === "ReviewProposals"){
          this._serviceResponseKey = "orderProposal";
          responseKey = this._serviceResponseKey;
        }
        else{
          responseKey = this._serviceResponseKey;
        }
...
//         this.setContext(this.response);
        ...
        if(this._topFlex == "Holdings"){
          ...
          if(scope._referenceCurrency && (listResponse[scope._referenceCurrency] === undefined || listResponse[scope._referenceCurrency] === "")){
            listResponse[scope._referenceCurrency] = objArr.response.referenceCurrency?objArr.response.referenceCurrency : "USD";
          } 
          ...
          this.view.lblRecommendation.text = kony.i18n.getLocalizedString("i18n.wealth.recommendProposal");
          this.view.lblProposalDate.text = kony.i18n.getLocalizedString("i18n.wealth.proposalDatemb");
          this.view.lblReviewTotalValue.text = kony.i18n.getLocalizedString("i18n.wealth.reviewTotalmb");
          this.view.lblFeesAndTaxValue.text = kony.i18n.getLocalizedString("i18n.wealth.feesAndTaxmb");
          var navManager = applicationManager.getNavigationManager();
          let reviewcurrency = this.fullResponse.orderAmountCurrency;
          if(this._topFlex === "ReviewPastProposals"){
          this.view.lblOrderTypes.text = this.fullResponse.pastProposal.length + " " + kony.i18n.getLocalizedString("i18n.wealth.orderTypesAvaliable");
        }
        else{
          this.view.lblOrderTypes.text = this.fullResponse.orderProposal.length + " " + kony.i18n.getLocalizedString("i18n.wealth.orderTypesAvaliable");
        }
          var reviewformattedAmt = this.formatUtils.formatText(this.fullResponse.totalOrderAmount, "Amount", this.formatSkins, this.formattingJSON, reviewcurrency );
          var reviewfeesandTax = this.formatUtils.formatText(this.fullResponse.totalFeesTaxes, "Amount", this.formatSkins, this.formattingJSON, reviewcurrency );
          this.view.lblProposalDate.text = this.getFieldValue(scope._proposalDate);
          if(this._topFlex === "ReviewProposals"){
          	this.view.lblProposalDateVal.text = this.fullResponse.proposaldateTime;
			this.view.lblProposalDate.text = this.fullResponse.proposaldateTime?kony.i18n.getLocalizedString("i18n.wealth.proposalDatemb"):"";
          }
          else if(this._topFlex === "ReviewPastProposals"){
            var dateproposal = navManager.getCustomInfo("frmPastProposalDate");
            this.view.lblProposalDateVal.text = dateproposal;
          }
          this.view.lblReviewTotalValue.text = this.getFieldValue(scope._reviewTotal);
          this.view.lblReviewTotalVal.text = reviewformattedAmt.text;
          this.view.lblFeesAndTaxValue.text = this.getFieldValue(scope._feesAndTax);
          this.view.lblFeesAndTaxVal.text = reviewfeesandTax.text;
		  this.view.lblFeesAndTaxValue.text = reviewfeesandTax.text?this.getFieldValue(scope._reviewTotal):"";
        }
          this.refCurrency = listResponse[scope._referenceCurrency];
        ...
        modifySegmentData: function() {
          ...
          var formUtilityMan = applicationManager.getFormatUtilManager(); 
            ...
            for (var i in this.response) {
              ...
              if(this._topFlex === "ReviewProposals" || this._topFlex === "ReviewPastProposals")
                {
                  var ref = "referenceCurrency";
                  var refvalVal = this.refCurrency;
				  resObj[ref] = refvalVal;
                }
                var skinId = "";
                let valueObj;
              ...
              record["lblId"] = {
                ...
                record["lblTwoKey"] = {
                    "text": this.getFieldValue(scopeObj._fieldTwoKey)==="Qty."?"Qty:":this.getFieldValue(scopeObj._fieldTwoKey),
                    "skin": scopeObj._allFieldKeySkin,
              ...
              if("orderReference" in resObj) {
                ...
                if(this._topFlex === "ReviewProposals" || this._topFlex === "ReviewPastProposals"){
                    record["lblFourKey"].text = kony.i18n.getLocalizedString("i18n.wealth.currentWeight");
                    ...
                    //var fourval = record["lblFourValue"].text.split(".");
                  record["lblFourValue"].text = record["lblFourValue"].text + "%";
				  valueObj.value = (resObj[valueObj.value])?"currentWeight":"";
                  record["lblFourKey"].text = (resObj[valueObj.value])?record["lblFourKey"].text:"";
                  }
                    else{
                  record["lblFourValue"] = this.formatUtils.formatText(resObj[valueObj.value], scopeObj._fieldFourType, this.formatSkins, this.formattingJSON, valueObj.currency);
                  } 
                ...
                valueObj = this.splitValueField(scopeObj._fieldFiveValue, resObj);
                //record["lblFiveValue"] = this.formatUtils.formatText(resObj[scopeObj._fieldFiveValue], scopeObj._fieldFiveType, this.formatSkins, this.formattingJSON, resObj[scopeObj._currencyCode]);
                 if(this._topFlex == "Transactions" && this._topFlex !== "ReviewProposals" && this._topFlex !== "ReviewPastProposals"){
                ...
                if(this._topFlex === "ReviewProposals" || this._topFlex === "ReviewPastProposals"){
                  record["lblFiveKey"].text = kony.i18n.getLocalizedString("i18n.wealth.targetWeight");
                    record["lblFiveValue"] = this.formatUtils.formatText(resObj[valueObj.value], scopeObj._fieldFiveType, this.formatSkins, this.formattingJSON, valueObj.currency);
                  //var fiveval = record["lblFiveValue"].text.split(".");
                  record["lblFiveValue"].text = record["lblFiveValue"].text + "%";
				  valueObj.value = (resObj[valueObj.value])?"targetWeight":"";
                  record["lblFiveKey"].text = (resObj[valueObj.value])?record["lblFiveKey"].text:"";
                  }
                    //record["lblFiveValue"].isVisible = (scopeObj._fieldFiveValue !== "") ? true : false;
                record["lblFiveValue"].isVisible = (valueObj.value !== "") ? true : false;
                skinId = record["lblFiveValue"].skin;
              ...
              if(this._topFlex != "Watchlist" && this._topFlex !== "ReviewProposals" && this._topFlex !== "ReviewPastProposals"){
              ...
              if(record["lblFourKey"].isVisible == true){
               ...
               if(record["lblOneKey"].isVisible == true)
                  record["lblOneKey"].right = (record["lblOneValue"].text.length < 11) ? "27%" : "30%";
              if(record["lblThreeKey"].isVisible == true){
                 if(this._topFlex == "Transactions"){
                   var len=record["lblThreeValue"].text.length;
                    if(len<5)
                      record["lblThreeKey"].right = "13%";
                    else if(len>5 && len<=9)
                      record["lblThreeKey"].right ="22%";
                   else if(len>9 && len<13)
                      record["lblThreeKey"].right ="25%";
                    else 
                      record["lblThreeKey"].right = "34%"; //IW-3822 Bharath
                 }
//                 if(this._topFlex == "Watchlist")
//                   record["lblThreeKey"].right = (record["lblThreeValue"].length < 11) ? "25%" : "30%";
//                   else 
                   else
                      record["lblThreeKey"].right = (record["lblThreeValue"].text.length < 11) ? "25%" : "32%";
              }
                if(record["lblFiveKey"].isVisible == true){
                    var len=record["lblFiveValue"].text.length;
                    if(len<9)
              ...
              if(record["lblTwoKey"].isVisible == true){
                record["lblTwoValue"].left = (record["lblTwoKey"].text.length < 7) ? "70dp" : ((record["lblTwoKey"].text.length >11) ? "110dp" : "78dp");
              }
              if(record["lblFourKey"].isVisible == true){
                record["lblFourValue"].left = (record["lblFourKey"].text.length < 7) ? "58dp" : ((record["lblFourKey"].text.length >11) ? "130dp" : "68dp");
              }
               if(record["lblOneKey"].isVisible == true)
                  record["lblOneKey"].right = (record["lblOneValue"].text.length < 11) ? "25%" : "30%";
              if(record["lblThreeKey"].isVisible == true){
                  record["lblThreeKey"].right = (record["lblThreeValue"].text.length < 2) ? "8%" : (record["lblThreeValue"].text.length < 3) ? "10%" : (record["lblThreeValue"].text.length < 5)? "13%" : "16%";
              }
                if(record["lblFiveKey"].isVisible == true){
					record["lblFiveKey"].right = (record["lblFiveValue"].text.length < 3) ? "10%" : (record["lblFiveValue"].text.length < 6) ? "17%" : (record["lblFiveValue"].text.length < 7) ? "18%" : "20%";
                  }
              }
				else{
				}
      ...
      imgArrowChange:function(){
        try{
        if(this.view.imgArrowProposal.src == "arrowup.png"){
          this.view.imgArrowProposal.src = "arrowdown.png";
          this.view.segList.setVisibility(false);
        }
        else if(this.view.imgArrowProposal.src == "arrowdown.png"){
          this.view.imgArrowProposal.src = "arrowup.png";
          this.view.segList.setVisibility(true);
        }
        }catch(err) {
        this.setError(err, "checkPermission");
        ...
        toggleOpenOrder: function(event, context) {
          try{
            ...
            if (existingparam !== "" || existingparam !== undefined) {
                ...
                delete existingparam.startDate;
                delete existingparam.endDate;
                this.setContext(existingparam);
                this.getWealthList();
            }
