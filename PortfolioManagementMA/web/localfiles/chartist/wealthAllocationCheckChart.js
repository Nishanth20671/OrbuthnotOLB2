function drawWealthAllocationChart(labelsArray, seriesArray,chartColors, strokewidth=false, offSet=false) {
...
if(offSet){
	if(seriesArray[0].length >4 ){
		options.axisY.offset = 110;
	}
	}
...
//IW-3681 Fix			
      ...
      let tot=x-(width/2);
      data.element._node.setAttribute('style', 'x: ' + tot + '; align-items: flex-end ; justify-content: center ; text-align: left ; text-anchor: start;');//iw-3782
        }
      ...
      if (chartColors[data.index]) {
        data.element._node.setAttribute('style', 'stroke: ' + chartColors[data.index] + '; stroke-width: 10px; ');
      }
    }
				}
      ...
      if (chartColors[data.seriesIndex]) {
        data.element._node.setAttribute('style', 'stroke: ' + chartColors[data.seriesIndex] + '; stroke-width: 10px; ');
				...
				if (data.type !== 'grid') {
      if (chartColors[data.index]) {
        data.element._node.setAttribute('style', 'stroke: ' + chartColors[data.index] + '; stroke-width: 20px; ');
      }
    }
				}
					...
					if (data.type !== 'grid') {
      if (chartColors[data.seriesIndex]) {
        data.element._node.setAttribute('style', 'stroke: ' + chartColors[data.seriesIndex] + '; stroke-width: 20px; ');
      }
    }
				}
				}
				...
				data.group.append(
					...
					height:2,
					opacity:".3",
					style: strokewidth ? "stroke:lightgrey; fill:lightgrey; stroke-width:10;":"stroke:lightgrey; fill:lightgrey; stroke-width:20;" 
					}, 'greyBG')
				)
//IW-3681 Fix
			//	if (value !== '0') {
