...
          visibility: true
    ...
    this.iPhoneMenuItems = this.isWatchlistEnabled ? this.iPhoneMenuItems : this.iPhoneMenuItems.filter(item => item.text !== "Watchlist");
...
//         },
       ...
       if (arrayItem["key"]) {
         switch (arrayItem["key"]) {
           ...
           case "INWARDCOLLECTIONS_STATUS":
             scope_configManager.inwardCollectionChartData = arrayItem["value"] ? JSON.parse(arrayItem["value"]) : [];
             break;
  ...
  */
  ...
  /**
	* get Inward CollectionsStatus Configuration
	*/
   ConfigurationManager.prototype.setInwardCollectionsStatus = function(data) {
	let Status = JSON.parse(data);
	scope_configManager.inwardCollectionsStatus = Status;
	};
	/**
  ...
  [constantName.TRADESUPPLYFINANCE]: 'BillsUIModule'
   };
   try{
